    ...
    public function ApiInputInteraction(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                $username = 'syifa';
                $password = 'infomedia';
                $response->status = DB::select("SELECT parameter FROM dwh_sources CROSS JOIN (SELECT '$ip' AS ip,'$username' AS username,'$password' AS password) params WHERE id = $id AND parameter @> jsonb_build_object('username',username) AND parameter @> jsonb_build_object('password',password) AND jsonb_exists(parameter->'allowed_ip', ip)");
                // if (count($source) === 1) {
